...
local function updateStaticPlayerTrainer(battle, mode)
  if not BattleArt.ownsPlayerTrainerArt() then
...
local function updatePlayerTrainer(battle, mode)
  if not BattleArt.ownsPlayerTrainerArt()
     or not (battle and battle.showPlayerBack and not battle.demo) then
    restoreTrainer(battle)
    return
  end
...
function AnimatedBattleArt.hasPlayerTrainerFrame(battle)
  local state = battle and trainerStates[battle]
  return BattleArt.ownsPlayerTrainerArt()
         and state and battle.playerBackPic == currentImage(state) or false
end

-- Public only inside the fused namespace.  The exported sprite-control seam
-- calls this before a live ownership change so the animation manager yields
-- immediately; identity-checked restoreTrainer leaves a later provider's
-- replacement untouched.
function AnimatedBattleArt.releasePlayerTrainer(battle)
