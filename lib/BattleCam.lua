...
-- The DRIFT's phase is session state, so every fight keeps the same gentle
-- breath. The opening orbit, pitch and lens are reset separately below.
--
-- Optional mapping helper for callers that explicitly want to place the
-- battle camera where the FREE-ROAM camera was last left. The free
...
function BattleCam.reset()
  -- A free-roam yaw/pitch and a third-person zoom describe different rigs and
  -- can even have been captured in different camera modes. Carrying that
  -- mixture into the long-lens battle rig can place its eye behind foreground
  -- terrain or outside the composition solved for the two Pokemon. Every new
  -- battle therefore starts on the authored shot. Steering and zoom remain
  -- live after entry; only the opening pose is reset.
  --
  -- Keep the drift phase (BattleCam.t) continuous. It is the shot's gentle
  -- parallax breath, not player-authored camera state.
