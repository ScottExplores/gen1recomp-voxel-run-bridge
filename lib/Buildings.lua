...
-- Tiles that are artwork for a building's FRONT only: entrances, shop
-- signs, and painted lettering.  The profile stores compact arrays; expose
-- them as sets so both this custom-building path and ChunkMesher's fallback
-- volume path make the same decision.
...
-- Map pixels belonging to front-only tiles to pixels from a neighbouring
-- ordinary wall tile in the same tile row.  The facade is extruded through
-- the footprint, so without this substitution the rear wall repeats the
-- front door, MART/POKe signs, and GYM lettering backwards.
--
-- Donors are selected per contiguous run of front-only cells.  This keeps
-- a two-cell doorway on one coherent wall material instead of borrowing a
-- different tile from either side.
local function backMap(tiles, bw, bh, W, inside, frontOnly)
  ...
  for r = 1, bh do
    ...
    while c <= bw do
      if frontOnly[row[c]] then
        ...
        local left, right = c - 1, c1 + 1
        local pick = nil
        if left >= 1 and right <= bw then
          pick = ((freq[row[right]] or 0) > (freq[row[left]] or 0))
                 and right or left
        elseif left >= 1 then
          pick = left
        elseif right <= bw then
          pick = right
        end
        if pick then
  ...
  local back = {}
  for cell, donorColumn in pairs(donor) do
    local r, c = math.floor(cell / bw), cell % bw
    for oy = 0, 7 do
      local sy = r * 8 + oy
      for ox = 0, 7 do
        local i = sy * W + c * 8 + ox
        local j = sy * W + donorColumn * 8 + ox
        -- Door thresholds reach the ground row while the adjoining wall
        -- may end one row above it.  Lift the donor exactly as model() lifts
        -- a building's base course before giving up on the substitution.
...
local function model(sp, pr, t)
  ...
  -- The back layer normally repeats the front drawing.  Replace only the
  -- pixels the profile marks as facade-only; ordinary wall and window art
