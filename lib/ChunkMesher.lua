...
-- Nearest horizontal donor columns for a rear/flank face whose own map
-- tile is marked as front-only building art.
local SPAN = { { -1, 1 }, { -2, 2 } }
...
local function newSink()
  -- Build directly into a live GPU upload sink.  This private fusion never
  -- serializes the stream or reloads a previous session's vertices.
...
local function runGeometry(map, bodyOnly, masks, sink, waterSink)
  ...
  -- Doors, MART/POKe signs, and GYM lettering belong only to the drawn
  -- south facade, so rear and flank faces borrow an ordinary wall tile from
  -- the same structure row.  The search remains inside the detected run;
  -- it can never borrow grass or another building's material.
  ...
  local function wallTile(tx, ty)
    ...
    for distance = 1, 2 do
      for _, offset in ipairs(SPAN[distance]) do
        local nx = tx + offset
        if S.runs[keyOf(nx, ty)] then
          local candidate = map:tileAt(nx, ty)
          if not frontOnly[candidate] then return candidate end
        end
      end
    end
    return tile
            ...
            for band = math.floor(nh / 8), math.ceil(h / 8) - 1 do
              ...
              if y1 > y0 then
                ...
                if run then
                  ...
                  -- The south face is the authored facade. Every other
                  -- face uses the same course with front-only art removed.
                  src = (d == 5) and map:tileAt(tx, sy)
                                 or wallTile(tx, sy)
      ...
      for _, q in ipairs(st.quads) do
        ...
        for i = 1, 4 do
          ...
          if st.lift and st.lift > 0 then
            ...
            if bases then
              local mapKey = map.id or (map.def and map.def.id)
                             or tostring(map)
              local baseKey = mapKey .. ":" .. st.mx .. "|" .. st.mz
              if not bases[baseKey] or c[2] < bases[baseKey] then
                bases[baseKey] = c[2]
              end
            end
          end
...
local jobFailures = {} -- last completed failure, for title-generator diagnosis
...
local function finishJob(job, ok, err)
  ...
  if not ok then
    jobFailures[key] = tostring(err or "unknown error")
...
-- for connected bodies, and false/nil for other live requests.
...
function ChunkMesher.pending() return #jobs end

function ChunkMesher.jobFailure(mapId, bodyOnly)
  return jobFailures[jobKey(mapId, bodyOnly and "body" or "full")]
end
...
-- from an invalidated slot while a transition decides whether to show its veil.
...
local IDLE_SLICE = 0.005 + ((function()
  local config = rawget(_G, "__ds_ceiling_config")
  config = config and config()
  return (config and config.fastchunks) and 0.007 or 0
end)())
...
-- Release this session's meshes and analysis for a reloaded map. This differs
