...
local function animationProjectionRecord()
  ...
  local record = rawget(BattleState, ANIMATION_PROJECTION_HOOK)
  if type(record) == "table"
     and record.owner == ANIMATION_PROJECTION_OWNER then
    return record
  end
  return nil
end

local function clearAnimationProjection(expectedBattle, releaseCanvas)
  local record = animationProjectionRecord()
  if not record
     or (expectedBattle ~= nil and record.battle ~= expectedBattle) then
    return
  end
  record.surface = nil
  record.battle = nil
  if releaseCanvas and record.canvas then
    if record.canvas.release then pcall(record.canvas.release, record.canvas) end
    record.canvas = nil
  end
end
...
function OverworldBattle.backPinned(activeBattle)
  if not OverworldBattle.enabled() then return false end
  local battle = activeBattle or (session and session.battle)
...
function OverworldBattle.begin(state, battle)
  ...
  session = { state = state, arena = arena, battle = battle, shot = nil,
              textures = nil, armed = false, token = 0 }
  cullCast(state)
  BattleCam.reset()
  return true
end
...
-- The physical arena and its floor height for the capture trajectory.
...
-- The rendered enemy card used to measure the visible body/collision mask.
...
function OverworldBattle.finish()
  -- The animation projection record outlives a module entry so an F5 in the
  -- middle of a fight can hand the live surface to the replacement export.
  -- Battle end is the lifetime boundary for that persistent state even when
  -- this freshly loaded module did not own the original staging session.
  clearAnimationProjection(nil, false)
...
function OverworldBattle.update(dt)
  ...
  -- CatchThrow keeps the deterministic alpha mask it snapped when the throw
  -- began, but the visible Pokemon should continue playing its selected
  -- animation. Presentation no longer freezes merely because collision owns
  -- a stable reference pose.
...
-- The staged battle's attack sprites are authored in the same 160x144 space
-- as the classic battle screen. Normally the engine composites that layer
-- over worldOverride. A dual-screen presenter can instead route uiCanvas to
-- another display, so publish the attack-only surface separately along with
-- the exact letterbox transform that places it over the window-sized arena.
-- Placement fields are copied; callers never receive the live session table.
function OverworldBattle.animationSurface(expectedBattle)
  local record = animationProjectionRecord()
  if not record
     or (expectedBattle ~= nil and record.battle ~= expectedBattle) then
    return nil
  ...
  if not (surface and surface.canvas and surface.canvas.getWidth) then
    return nil
  end
  return {
...
function OverworldBattle.invalidate()
  ...
  clearAnimationProjection(nil, true)
  if session then session.animationSurface = nil end
...
local function spriteOwnershipCallbacks()
  return {
    capture = function(battler)
      if not (BattleArt.ownsSpeciesArt() and battler) then return nil, nil end
      local image = BattleArt.isExternal(battler.sprite) and battler.sprite
                    or nil
      return image, BattleArt.speciesFor(battler)
    end,
    retain = OverworldBattle.retainOwnedSprite,
  }
end

-- Install once during ordinary setup and force one post-provider outer guard
-- at mods.loaded.  The record lives on BattleState rather than in this entry
-- chunk: F5 evaluates every module again, so a module-local wrapper marker
-- cannot see (or retire) the old closure.
--
-- Each node owns an immutable `inner`.  Replacing the active node first marks
-- it inactive, making every older wrapper an exact multi-return passthrough;
-- the newest outer node alone calls the callbacks refreshed by the newest
-- entry load.  Never sharing a mutable `inner` also prevents recursion when a
-- provider has already wrapped an older Battle Art node.
function OverworldBattle.refreshSpriteOwnershipHook(postProviders)
  local BattleState = require("src.battle.BattleState")
  local record = rawget(BattleState, SPRITE_OWNERSHIP_HOOK)
  if type(record) ~= "table"
     or record.owner ~= SPRITE_OWNERSHIP_OWNER then
    record = {
      owner = SPRITE_OWNERSHIP_OWNER,
      installCount = 0,
      generation = 0,
    }
    rawset(BattleState, SPRITE_OWNERSHIP_HOOK, record)
  end

  -- A reload that reaches this point immediately makes every still-active
  -- dispatcher use the new BattleArt/OverworldBattle closures, even before a
  -- provider has finished loading and the final outer node is installed.
  record.callbacks = spriteOwnershipCallbacks()
  local active = record.active
  -- Only the first-ever entry needs a pre-provider node.  On F5 the existing
  -- post node can dispatch the freshly replaced callbacks immediately; adding
  -- another pre node would leave one more inert closure in the chain on every
  -- reload.  The later post-provider refresh will add one outer node only if
  -- a provider actually changed BattleState.update in the meantime.
  if active and active.active and not postProviders then return false end

  local phase = postProviders and "post" or "pre"
  if active and active.active and active.phase == phase
     and BattleState.update == active.wrapper then
    return false
  end

  if active then
    active.active = false
    record.lastRetired = active
  end
  ...
  local node = {
    ...
    phase = phase,
    active = true,
    inner = BattleState.update,
    generation = (record.generation or 0) + 1,
  }
  node.wrapper = function(self, dt, ...)
    if not node.active or record.active ~= node then
      return node.inner(self, dt, ...)
    end

    local callbacks = record.callbacks
    local enemy = self and self.enemy
    local player = self and self.player
    local ownedEnemy, enemySpecies = callbacks.capture(enemy)
    local ownedPlayer, playerSpecies = callbacks.capture(player)
    local results = packValues(node.inner(self, dt, ...))
    callbacks.retain(enemy, ownedEnemy, enemySpecies)
    callbacks.retain(player, ownedPlayer, playerSpecies)
  ...
  record.active = node
  BattleState.update = node.wrapper
  BattleState.dramaticShapeDuplicateFixHook = true
  return true
end

-- Project the engine's authored OAM move layer into the staged battlers'
-- current screen-space marks, then retain a transparent copy for a presenter
-- that routes the ordinary UI canvas to another display.  This function is
-- deliberately module-owned: the persistent dispatcher below replaces its
-- callback on every entry load, while the live surface itself remains on the
-- engine-owned record so a mid-battle F5 can adopt it immediately.
local function drawProjectedAnimation(self, innerAnim, colorized, record)
  local shot = self.dramaticShapeShot
  if not shot then
    if record and record.battle == self then
      record.surface = nil
      record.battle = nil
    end
    if session and session.battle == self then session.animationSurface = nil end
    return innerAnim(self, colorized)
  end
  ...
  --
  -- And they ride the pair's SEPARATION as well, because the mons themselves
  -- do. Scaling about the same midpoint keeps every authored offset the same
  -- fraction of the projected gap it was authored against.
  local a = OverworldBattle.ANCHOR
  local px, py = shot.player[1], shot.player[2]
  if OverworldBattle.backPinned(self) then px, py = a.player[1], a.player[2] end
  ...
  -- add a placement transform outside this dispatcher. Rendering the
  -- engine's inner draw a second time is safe (AnimPlayer:draw mutates no
  -- battle state) and excludes menus, text, and HUD chrome.
  local active = self.animPlayer and (self.animPlaying or self.lockedBall)
  if not active then
    if record.battle == self then
      record.surface = nil
      record.battle = nil
    end
    if session and session.battle == self then session.animationSurface = nil end
  ...
  if not record.canvas then
    local okCanvas, made = pcall(love.graphics.newCanvas,
      BattleScene.GB_W, BattleScene.GB_H, { dpiscale = 1 })
    if okCanvas and made then
      record.canvas = made
  ...
  local pushed = false
  if g.push then g.push("all"); pushed = true end
  local okCapture, captureErr = pcall(function()
  ...
  if not okCapture then
    ...
    record.battle = nil
    if session and session.battle == self then session.animationSurface = nil end
    if not record.surfaceWarned then
      record.surfaceWarned = true
      V.mod.log:warn("staged battle animation surface failed: %s",
                     tostring(captureErr))
    end
    return
  end
  ...
  if session and session.battle == self then session.animationSurface = surface end
end

-- BattleState persists across a development reload while this module and its
-- `session` upvalue do not.  A boolean "already wrapped" marker therefore
-- strands drawAnimLayer in the first entry's closure.  Keep one dispatcher on
-- the engine class and refresh only its module callback.  A later sprite mod
-- may remain wrapped outside it; the stable dispatcher identity means that
-- provider chain does not grow on each F5.
function OverworldBattle.refreshAnimationProjectionHook()
  local BattleState = require("src.battle.BattleState")
  local record = rawget(BattleState, ANIMATION_PROJECTION_HOOK)
  if type(record) ~= "table"
     or record.owner ~= ANIMATION_PROJECTION_OWNER then
    record = {
      owner = ANIMATION_PROJECTION_OWNER,
      inner = BattleState.drawAnimLayer,
      installCount = 0,
      generation = 0,
    }
    record.wrapper = function(self, colorized)
      local callback = record.callback
      if type(callback) == "function" then
        return callback(self, record.inner, colorized, record)
      end
      return record.inner(self, colorized)
    end
    rawset(BattleState, ANIMATION_PROJECTION_HOOK, record)
  end

  record.callback = drawProjectedAnimation
  record.generation = (record.generation or 0) + 1
  if BattleState.drawAnimLayer == record.inner then
    BattleState.drawAnimLayer = record.wrapper
    record.installCount = (record.installCount or 0) + 1
    return true
  end
  if BattleState.drawAnimLayer == record.wrapper then return false end

  -- Another provider owns the outer method and is expected to call the
  -- stable dispatcher it wrapped. Do not jump outside or duplicate it.
  return false
end
