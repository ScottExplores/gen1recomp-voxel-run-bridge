...
function BattleStage.export(battles)
  ...
  local function animationSurface(expectedBattle)
    local surface = call(battles, "animationSurface", expectedBattle)
    if type(surface) ~= "table"
       or not (surface.canvas and surface.canvas.getWidth) then
      return nil
    end
    return {
      apiVersion = BattleStage.API_VERSION,
      ...
      lx = tonumber(surface.lx) or 0,
      ly = tonumber(surface.ly) or 0,
      scale = tonumber(surface.scale) or 1,
      pw = tonumber(surface.pw),
      ph = tonumber(surface.ph),
    }
  end
