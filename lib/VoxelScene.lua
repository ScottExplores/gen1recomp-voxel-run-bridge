-- Voxel world mode: assemble and draw each frame of the 3D scene.
--
...
-- Kanto First Person is part of this package.  Each optional presentation
-- module stays fail-open so one effect cannot take down the base world.
local function kantoModule(name, statusKey)
  local ok, value = pcall(V.require, name)
  if ok and type(value) == "table" then return value end
  _G[statusKey] = name .. " load failed: " .. tostring(value)
  return setmetatable({}, {
    __index = function() return function() end end,
  })
end
local Ceiling = kantoModule("Ceiling", "__ds_ceiling_status")
local Backdrop = kantoModule("Backdrop", "__ds_backdrop_status")
local SkyLayer = kantoModule("SkyLayer", "__ds_sky_status")
local Flora = kantoModule("Flora", "__ds_flora_status")
...
function VoxelScene.prefetch(state)
  ...
  for i, nb in ipairs(nbs) do
    -- Every voxel camera needs the next body ready before its seam. Build all
    -- connected bodies on neighbourhood entry, but withhold them from every
    -- draw pass until neighbor() admits them under the selected R.DIST.
    if RenderDistance.prefetchNeighbor(nb) then
      ChunkMesher.request(nb.map, true, nil,
                          RenderDistance.prefetchPriority(nb, state.player))
    end
...
local function castShadows(state, terrain, nbMesh, posed, cx, cy, vw, vh,
  ...
  -- Tall-tree trunks, rock supports and their hoods are separate geometry,
  -- so explicitly add them to the sun pass as well as the colour pass.
  pcall(function()
...
function VoxelScene.render(state, w, h, vw, vh, paletteFor)
  ...
  -- Interiors, cutaways, trunks, boulders and ground detail share the same
  -- depth-tested scene as Battle Art's terrain.
