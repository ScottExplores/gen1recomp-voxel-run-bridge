          ...
          if data then
            ...
            S.roundStamps[#S.roundStamps + 1] =
              { quads = tpl.quads, mx = cx * 16 + 16, mz = cy * 16 + 16,
                r = 16,
                -- A grouped four-cell round supersedes its member cells.
                _kantoTomb = (function()
                  local registry = rawget(_G, "__ds_round_cells")
                  local mapKey = map.id or (map.def and map.def.id)
                                 or tostring(map)
                  if registry and registry[mapKey] then
                    for dy = 0, 1 do
                      for dx = 0, 1 do
                        registry[mapKey][(cx + dx) .. "|" .. (cy + dy)] = nil
                      end
                    end
                  end
                end)() }
        ...
        if data then
          ...
          S.roundStamps[#S.roundStamps + 1] =
            { quads = tpl.quads, mx = cx * 16 + 8, mz = cy * 16 + 8,
              -- Publish a deterministic lift only for real cylinder stamps.
              -- Flora uses this registry to build trunks/rock supports under
              -- the lifted canopy; it is repopulated on every geometry build.
              lift = (function()
                local config = rawget(_G, "__ds_ceiling_config")
                config = config and config()
                if not (config and config.talltrees
                    and s.class == "cylinder") then return 0 end
                local lift = 8 + ((cx * 73856093 + cy * 19349663) % 3) * 6
                local registry = rawget(_G, "__ds_round_cells")
                local mapKey = map.id or (map.def and map.def.id)
                               or tostring(map)
                if registry then
                  registry[mapKey] = registry[mapKey] or {}
                  registry[mapKey][cx .. "|" .. cy] = lift
                end
