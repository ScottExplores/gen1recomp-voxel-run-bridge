...
-- own part of the frame. This build registers three:
--
...
--
--   lavveil    an automatic Lavender fog veil supplied by the fused Kanto
--              presentation. It is a pass-through everywhere else.
--
...
-- Kanto First Person is fused into this private build instead of running as
-- a companion patcher.  Publish its configuration before renderer modules are
-- evaluated: Structures, ChunkMesher, Ceiling and Flora all consult it while
-- constructing their first map.
local ScottKanto = V.require("ScottKanto")
ScottKanto.bootstrap()
...
-- Exact Dramatic Shape catching: LetsGo owns the modes and engine seams;
-- Pokeball owns the physical prop rendered in the staged battle scene.
...
-- wrapping the registries, so Battle Art can reassert sprite ownership after
-- other sprite providers have installed their update hooks.
mod.events:on("mods.loaded", function()
  ...
  OverworldBattle.refreshSpriteOwnershipHook(true)
end)
...
mod.content.render_pipelines:register("voxel", {
  ...
  update = function(dt, level)
    ...
    -- would mid-walk
    DayNight.update(dt)
    -- Pose the capture session before the arena is rendered so this frame's
    -- ball is exactly the one this frame computed. A capture failure is
    -- isolated from the rest of the voxel pipeline and reported once.
    do
  ...
  invalidate = function()
    ...
    Pokeball.invalidate()      -- ball meshes and palette texture
  end,
})
...
local SETTINGS = {
  ...
  { OverworldBattle.setting,
    ...
    full = true },
  -- Gameplay, not a diorama look knob: the VOXEL FULL preset neither sets
  -- this row nor hides it. Exact upstream semantics default to OFF.
  { LetsGo.setting,
  ...
  { BattleArt.setting,
    ...
    .. "back to the ROM. ANIMATED is the default.",
    when = function() return stagedBattles() end, full = true },
  { BattleArt.opponentTrainerSourceSetting,
    "Choose who owns the opponent trainer portrait. BATTLE ART uses the "
    .. "TRAINER ART generation below; MODDED yields the field to another "
    .. "sprite provider without restoring over a newer image.",
    when = function() return stagedBattles() end, full = true },
  { BattleArt.playerTrainerSourceSetting,
    "Choose who owns the player trainer intro portrait. BATTLE ART uses "
    .. "PLAYER ART or PLAYER ANIM below; MODDED yields both static and "
    .. "animated portraits to another sprite provider.",
    when = function() return stagedBattles() end, full = true },
  { BattleArt.trainerSetting,
    ...
    when = function()
      return stagedBattles() and BattleArt.setting:get() ~= "rom"
             and BattleArt.ownsOpponentTrainerArt()
    end, full = true },
  { BattleArt.playerArtSetting,
    ...
    when = function()
      ...
      return stagedBattles() and BattleArt.ownsPlayerTrainerArt()
             and (mode == "static" or mode == "rom")
    end, full = true },
  { BattleArt.playerAnimationSetting,
    ...
    when = function()
      return stagedBattles() and BattleArt.ownsPlayerTrainerArt()
             and BattleArt.setting:get() == "animated"
    end, full = true },
...
-- The fused Kanto rows live in the same schema and therefore appear directly
-- on this mod's settings page.  ScottKanto deliberately contributes no second
-- shadow row; its presentation effects read Battle Art's shadowQuality key.
...
-- One step of the VOXEL camera ladder, shared by the keyboard's 3 and the
-- pad's SELECT hook below. Keeping the complete step here makes both inputs
-- use the registry's free-roam gate, persistence, and TILT/GBC FX cleanup.
...
mod.hooks:wrap("ui.options.rows", function(next, game, rows)
  ...
  dropRow(out, "tilt")
  dropRow(out, "gbcfx")
  -- The fused Lavender pass has its canonical home under WORLD. Keeping a
  -- second engine-generated row here would split one setting across menus.
  dropRow(out, "pipeline:lavveil")
  ...
  -- with the complete schema divided among six ordinary OptionRows screens;
  -- VOXEL and T-SHIFT stay here as the two immediate display-mode controls.
  return insertGrouped(out, { SettingsMenu.optionsRow() })
end)

-- Scott's fused build intentionally has no title-screen preload, no start
-- menu cache control and no disk/session mesh persistence.  Ordinary bounded
-- ChunkMesher streaming remains active, so every geometry build also reruns
-- the Kanto tree/rock support registration that the renderer consumes.
...
-- SELECT walks the same camera ladder as 3, but only from the overworld's
-- own input choke point. Menus and dialogs therefore retain SELECT, and a
-- refused mid-transition/cutscene press is forwarded unchanged. This wrap
-- must sit outside FreeMove's so 1ST/3RD can always be stepped off.
--
-- The marker is an owned table rather than a boolean: re-running this entry
-- during a development hot reload refreshes the shared cycle closure without
-- nesting a second wrapper or disturbing a later mod wrapped around it.
do
  local OverworldState = require("src.world.OverworldController")
  local SELECT_HOOK = "battleArtVoxelSelectHook"
  local hook = rawget(OverworldState, SELECT_HOOK)

  if type(hook) == "table" and hook.owner == mod.id then
    hook.cycle = cycleVoxel
    if OverworldState.handleInput == hook.inner then
      OverworldState.handleInput = hook.wrapper
    end
  elseif hook == nil then
    hook = {
      owner = mod.id,
      inner = OverworldState.handleInput,
      cycle = cycleVoxel,
    }
    hook.wrapper = function(self, ...)
      ...
      if input and input.wasPressed and input:wasPressed("select") then
        if hook.cycle(Game) then return end
      end
      return hook.inner(self, ...)
    end
    rawset(OverworldState, SELECT_HOOK, hook)
    OverworldState.handleInput = hook.wrapper
  end
end
...
-- Register the categorized screens after the ordinary options_changed bridge
-- above, so a screen write first synchronizes every ModSetting cache and only
-- then asks the integrated Crystal/provider controller to reconcile ownership.
SettingsMenu.install(schema)

-- Installed after the battle staging and pointer owners so capture is the
-- outer consumer only while a throw is being aimed. It also installs the
-- bag/Safari interception and FULL-mode experience hooks.
LetsGo.install()
...
-- Battle Art historically defaulted WORLD FILL to CYAN.  In the fused build
-- that plane sits in front of Kanto First Person's deeper apron and can hide
-- the intended horizon.  Migrate an absent/old-CYAN value to OFF/KFP once,
-- while preserving BLACK or any later choice the player makes.
local WORLD_FILL_MIGRATION = "_scottKfpWorldFill1"
local LAVENDER_VEIL_MIGRATION = "_scottLavVeilPipeline1"
local function migrateWorldFill(save)
  ...
  if bucket[WORLD_FILL_MIGRATION] then return false end

  local previous = bucket.worldFill
  if previous == nil or previous == "cyan" then bucket.worldFill = "off" end
  bucket[WORLD_FILL_MIGRATION] = true
  ...
  if loader then
    ...
    loader.modOptions[mod.id].worldFill = bucket.worldFill
    loader.modOptions[mod.id][WORLD_FILL_MIGRATION] = true
  end
  WorldUnderlay.setting:sync(bucket.worldFill or "off")
  if Game and Game.writeOptions then pcall(Game.writeOptions, Game) end
  return true, bucket.worldFill, previous
end

-- The original fused Lavender pipeline declared only { "AUTO" }. The engine
-- correctly interpreted that as a one-label ladder whose maximum level was
-- zero, so every pre-fix save could only store an unusable OFF value. Enable
-- the repaired OFF/AUTO ladder once, then retain every later explicit choice.
local function migrateLavenderVeil(save)
  local options = save and save.options
  if type(options) ~= "table" then return false end
  options.pipelines = type(options.pipelines) == "table"
    and options.pipelines or {}
  options.modOptions = type(options.modOptions) == "table"
    and options.modOptions or {}
  local bucket = options.modOptions[mod.id]
  if type(bucket) ~= "table" then
    bucket = {}
    options.modOptions[mod.id] = bucket
  end
  if bucket[LAVENDER_VEIL_MIGRATION] == true then return false end

  local previous = options.pipelines.lavveil
  if previous == nil or tonumber(previous) == 0 then
    options.pipelines.lavveil = 1
  end
  bucket[LAVENDER_VEIL_MIGRATION] = true
  ...
  if loader then
    loader.modOptions = loader.modOptions or {}
    loader.modOptions[mod.id] = loader.modOptions[mod.id] or {}
    loader.modOptions[mod.id][LAVENDER_VEIL_MIGRATION] = true
  end
  if Game and Game.writeOptions then pcall(Game.writeOptions, Game) end
  return true, options.pipelines.lavveil, previous
end
...
mod.events:on("save.loaded", function(payload)
  ...
  migrateWorldFill(save)
  local voxelSeeded = save and Voxel.seedOptions(save.options) or false
  local lavenderMigrated = migrateLavenderVeil(save)
  if voxelSeeded or lavenderMigrated then
...
mod.events:on("save.created", function(payload)
  ...
  migrateWorldFill(save)
  if save then
    Voxel.seedOptions(save.options)
    migrateLavenderVeil(save)
  end
  BattleArt.migrateDuplicateSetting()
  DayNight.restore()
  pinEngineFx()
...
mod.exports.spriteControl = SpriteControl.export()
mod.exports.settingsMenu = SettingsMenu.export()
mod.exports.migrateWorldFill = migrateWorldFill
mod.exports.migrateLavenderVeil = migrateLavenderVeil
mod.exports.letsGo = { mode = LetsGo.mode, combo = LetsGo.combo }
