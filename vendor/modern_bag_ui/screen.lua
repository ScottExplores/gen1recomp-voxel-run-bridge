...
return function(mod, priorBagMenu)
  ...
  -- VENDORED CHANGE (Scott's Tweaks): wrappers around lower screen owners
  -- retain every return value on both PUC Lua 5.1 and LuaJIT.
  local function pack(...)
    return { n = select("#", ...), ... }
  end
  ...
  local function classicSkin()
    return mod.options:get("skin") ~= "modern"
  end
  ...
  local function physicalThorLowerActive()
    ...
    if type(mod.find) == "function" then
      for _, id in ipairs({ "BATTLE_ART_VOXEL_FORK", "voxel_run_bridge" }) do
        local ok, found = pcall(mod.find, id)
        if (not ok or not found) then
          ok, found = pcall(mod.find, mod, id)
        end
        if ok and type(found) == "table" then
          candidates[#candidates + 1] = found
        end
      end
    end
    for _, handle in ipairs(candidates) do
      local exports = type(handle.exports) == "table" and handle.exports
        or nil
      local thor = exports and exports.thorDualScreen
      if type(thor) == "table" and type(thor.getEnabled) == "function"
          and type(thor.secondDisplayAttached) == "function" then
        local okEnabled, enabled = pcall(thor.getEnabled)
        local okAttached, attached = pcall(thor.secondDisplayAttached)
        if okEnabled and okAttached and enabled == true and attached == true then
          return true
        end
      end
    end
  ...
  local function layoutFor(menu)
    ...
    if not physicalThorLowerActive() and renderer and renderer.uiSize then
  ...
  local function normalizedPocket(value)
    ...
    local aliases = {
      ...
      -- VENDORED CHANGE (Scott's Tweaks): the artwork has five compartments,
      -- so explicit battle-item metadata folds into Items instead of naming a
      -- sixth, unreachable category.
      battle = "items", battleitem = "items", battleitems = "items",
      tm = "machines", tms = "machines", hm = "machines",
      hms = "machines", tmhm = "machines", machine = "machines",
      machines = "machines",
  ...
  local function categoryFor(game, id)
    ...
    -- VENDORED CHANGE (Scott's Tweaks): retain the compatibility inference
    -- used by Scott's prior Pack for content mods that omit machine metadata.
    if tostring(id):match("^TM_") or tostring(id):match("^HM_") then
      return "machines"
    end
  ...
  -- Gen1 item definitions normally all occupy the native ITEM pocket, while
  -- Gen2-capable data and custom items may declare BALL, KEY_ITEM or TM_HM.
  -- Derive the actual native pocket behind each visual category from the
  -- loaded definitions instead of assuming either model. If one visual view
  -- mixes native pockets, it has no truthful single denominator and shows its
  -- own occupied stack count only.
  local function nativePocketForView(game, view)
    if type(Bag.pocketOf) ~= "function" then return nil end
    local found
    for id in pairs((game.data and game.data.items) or {}) do
      if not Bag.isBadge(id) and categoryFor(game, id) == view then
        local nativePocket = Bag.pocketOf(id, game.data)
        if found and nativePocket ~= found then return nil end
        found = nativePocket
      end
    end
    return found
  ...
  local function visibleStackCount(menu)
    local count = 0
    for _, row in ipairs(menu.items or {}) do
      if row.value and not Bag.isBadge(row.value) then count = count + 1 end
    end
    return count
  end

  local function capacityText(menu)
    local config = menu.modernBagListConfig
    if config and type(config.capacity) == "function" then
      return tostring(config.capacity(menu) or "")
    end
    local game = menu.game
    local pocket = pocketFor(menu)
    if pocket.key == "all" then
      return tostring(Bag.slots(game.save, game.data))
    end
    local nativePocket = nativePocketForView(game, pocket.key)
    if not nativePocket then return tostring(visibleStackCount(menu)) end
    return ("%d/%d"):format(
      Bag.slots(game.save, game.data, nativePocket),
      Bag.capacity(game.data, nativePocket))
  end
  ...
  local function drawHeader(menu, layout)
    ...
    -- active pocket count after the label can run directly into the native
    -- slot/capacity value once the Bag contains many unique items.
  ...
  local function drawClassicPocketBag(key, x, y, width, height)
    ...
    local by = math.floor(y + (height - bagH) / 2)

    local function block(region, rx, ry, rw, rh)
      gray(key == region and BLACK or WHITE)
      love.graphics.rectangle("fill", bx + rx, by + ry, rw, rh)
      gray(DARK)
      love.graphics.rectangle("line", bx + rx, by + ry, rw, rh)
    end

    -- Handle, shoulder straps and the shared pack body.
    gray(DARK)
    love.graphics.rectangle("fill", bx + 11, by, bagW - 22, 2)
    love.graphics.rectangle("fill", bx + 8, by + 2, 3, 4)
    love.graphics.rectangle("fill", bx + bagW - 11, by + 2, 3, 4)
    love.graphics.rectangle("fill", bx + 1, by + 7, 3, bagH - 8)
    love.graphics.rectangle("fill", bx + bagW - 4, by + 7, 3, bagH - 8)
    gray(LIGHT)
    love.graphics.rectangle("fill", bx + 4, by + 4, bagW - 8, bagH - 4)
    gray(DARK)
    love.graphics.rectangle("line", bx + 4, by + 4, bagW - 8, bagH - 4)

    -- Five non-overlapping selectable compartments: side utility pouches,
    -- upper body, center catch pouch and lower machine pocket.
    block("items", 0, 9, 6, bagH - 11)
    block("key", bagW - 6, 9, 6, bagH - 11)
    block("medicine", 9, 3, bagW - 18, 7)
    block("balls", 8, 11, bagW - 16, 3)
    block("machines", 9, 15, bagW - 18, bagH - 16)

    -- Small clasps make narrow center regions readable without category art.
    gray(key == "balls" and WHITE or BLACK)
    love.graphics.rectangle("fill", bx + math.floor(bagW / 2) - 1,
      by + 11, 2, 3)
    gray(key == "machines" and WHITE or BLACK)
    love.graphics.rectangle("fill", bx + math.floor(bagW / 2) - 1,
      by + bagH - 4, 2, 2)
    return true
  end
  ...
  local function decorateList(menu, config)
    -- VENDORED CHANGE (Scott's Tweaks): do not stack a second responsive
    -- controller when a lower screen owner already decorated this list.
    if type(menu) ~= "table" or menu.modernBagUI == true then return menu end
    ...
    menu.modernBagProvider = mod.id
    menu.modernBagUpstreamVersion = "0.4.1"
  ...
  return {
    ...
    new = function(game, opts)
      ...
      -- VENDORED CHANGE (Scott's Tweaks): preserve the complete controller
      -- supplied by the previously registered BagMenu owner. Only the rows,
      -- drawing and pocket navigation below are replaced.
      local menu = construct(priorBagMenu, BagMenu, game, opts)
      if type(menu) ~= "table" or menu.modernBagUI == true then return menu end
      ...
      menu.onChoose = function(item, list)
        ...
        local result = pack(baseChoose(item, list))
