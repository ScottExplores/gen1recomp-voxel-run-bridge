...
Config.DEFAULTS = {
  ...
  -- VENDORED CHANGE (Scott's Tweaks): upstream defaults to "immersed", which
  -- returns a downward tuck and relies on Dramatic Shape's native grass mesh to
  -- redraw over the feet. Under the bundled Battle Art voxel renderer the grass
  -- is real geometry, so a tucked sprite is occluded outright and wild Pokemon
  -- become invisible in grass while town and ambient ones -- which stand on no
  -- grass mesh -- still show. "above" applies a lift instead. Switchable in the
  -- menu; only the default changed.
  pokemon_grass_render_mode = "above",
